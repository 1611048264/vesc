...
void mc_interface_mc_timer_isr(bool is_second_motor) {
	...
	*/
	...
	if (conf_now->l_slow_abs_current) {
		...
		if (fabsf((float)(GET_CURRENT1()-2048))* FAC_CURRENT> conf_now->l_abs_current_max) {
			mc_interface_fault_stop(FAULT_CODE_ABS_OVER_CURRENT, is_second_motor, true);
		}
		if (fabsf((float)(GET_CURRENT2()-2048))* FAC_CURRENT> conf_now->l_abs_current_max) {
			mc_interface_fault_stop(FAULT_CODE_ABS_OVER_CURRENT, is_second_motor, true);
		}		
#ifdef HW_HAS_3_SHUNTS
		if (fabsf((float)(GET_CURRENT3()-2048))* FAC_CURRENT> conf_now->l_abs_current_max) {
			mc_interface_fault_stop(FAULT_CODE_ABS_OVER_CURRENT, is_second_motor, true);
		}
#endif
		if (fabsf(abs_current) > conf_now->l_abs_current_max) {
